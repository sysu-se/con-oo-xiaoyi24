<script>
	...
	import { grid, gameStore, invalidCells } from '@sudoku/stores/grid';
		...
		{#each $gameStore.grid as row, y}
			{#each row as value, x}
				<Cell {value}
				      ...
				      userNumber={$gameStore.given[y][x] === 0}
				      ...
				      conflictingNumber={$settings.highlightConflicting && $gameStore.given[y][x] === 0 && $invalidCells.includes(x + ',' + y)} />
				{/each}
			{/each}
